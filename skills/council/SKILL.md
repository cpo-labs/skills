...
| --- | --- |
| Breaking a feature into implementation steps | a planning approach like `blueprint` |
| Pressure-testing a half-formed plan | `plan-interview` |
| Reviewing code for bugs or security | a dedicated code-review pass |
...
A council is a thinking tool, not a logging tool. Do **not** scatter ad-hoc notes
into shadow paths just because a council ran.

Only persist a decision when it changes something real. When it does:
- record the lesson wherever the project already keeps durable decisions (for
  example an `elephant`-style memory folder, a design doc, or an ADR)
- or update the relevant GitHub / issue directly if the decision changes active
  execution truth

If the verdict only confirmed the existing direction, there is nothing to store.
...
- `plan-interview` — pressure-test a plan one question at a time before or after the call
- `blueprint` — turn the chosen path into a buildable plan
- `elephant` — persist a durable decision delta when the outcome changes real direction
...
```text
Should we ship 2.0 as alpha now, or hold until the control-plane UI is more complete?
```
